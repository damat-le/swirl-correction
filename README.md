...
### Architecture
The network proposed here is a residual network that gives the following output:
...
$$
E(x,y) = \frac{\operatorname{MSE}(\hat{y}(x),y)}{\operatorname{MSE}(x,y)} 
$$

where $\operatorname{MSE}$ denotes the Mean Squared Error, $x$ is the swirled image, $y$ is the ground truth image, and $\hat{y}(x)$ is the output of the network. The metric ranges from 0 to 1, where 1 means that the network is able to reconstruct the swirled image perfectly, and 0 means that the network is able to invert the swirl applied to the ground truth image.
